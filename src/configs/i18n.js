...
const i18nMap = {
  'zh-CN': {
    ...
    metric: '公制，摄氏度 (°C)',
    imperial: '英制，华氏度 (°F)',
    getWeatherError: (httpCode, apiCode, apiMessage) => `获取天气失败，HTTP 状态码: ${httpCode}${apiCode ? `, API 状态码: ${apiCode}` : ''}${apiMessage ? `, 错误消息: ${apiMessage}` : ''}。`,
    location: '位置',
    useGeoLoc: '使用定位',
    ...
    burnInHomogenize: '均匀老化',
    notRecommend: '不推荐',
    burnInTip: '每天特定时段显示变换的扫描线使屏幕均匀老化，以缓解“烧屏”现象，会加剧屏幕损耗。',
    burnInTime: '老化开始时间',
    burnInDuration: '老化持续时间',
  },
  'en-US': {
    settings: 'Settings',
    apiKey: 'API Key',
    ...
    metric: 'Metric (°C)',
    imperial: 'Imperial (°F)',
    getWeatherError: (httpCode, apiCode, apiMessage) => `Get weather failed，HTTP code: ${httpCode}${apiCode ? `, API code: ${apiCode}` : ''}${apiMessage ? `, error message: ${apiMessage}` : ''}.`,
    location: 'Location',
    ...
    windSpeed: speed => `${speed} m/s`,
    burnInHomogenize: 'Burn-in Protect',
    notRecommend: 'Not Recommend',
    burnInTip: 'Display moving scan lines at specific times of day to ease screen burn. This will make the screen age faster.',
    burnInTime: 'Burn-in start time',
    burnInDuration: 'Burn-in Duration',
  },
};

const i18n = new Proxy(i18nMap, {
  get(target, key, receiver) {
    return Reflect.get(target[store.state.settings.language], key, receiver);
  },
});
